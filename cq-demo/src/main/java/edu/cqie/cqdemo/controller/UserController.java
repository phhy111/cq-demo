...
import edu.cqie.cqdemo.dto.ResetPasswordDTO;
    ...
    public Result toggleUserCollection(@RequestBody Collections collection) {
        try {
            ...
            if (isCollected != null && isCollected) {
                ...
                // 设置7天过期时间
                redisUtil.expireWithJitter(redisKeyAdd, 7, TimeUnit.DAYS);
        ...
        try {
            ...
            long likeCount = likesService.count(
                    new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<Likes>()
                            .inSql("target_id", "SELECT id FROM routes WHERE user_id = " + userId.longValue())
                            .eq("target_type", 3)
            ) + likesService.count(
                    new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<Likes>()
                            .inSql("target_id", "SELECT id FROM guides WHERE user_id = " + userId.longValue())
                            .eq("target_type", 4)
            );
    ...
    public Result getUserWorks(@RequestParam Long userId, @RequestParam Integer status, @RequestParam(required = false) String type) {
        try {
            ...
            if (acquired != null && acquired) {
                ...
                // 其他请求正在处理，轮询等待结果（最多重试3次，每次间隔200ms）
                for (int i = 0; i < 3; i++) {
                    Thread.sleep(200);
