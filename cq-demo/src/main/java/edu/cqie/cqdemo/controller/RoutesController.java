...
public class RoutesController {
    ...
    // 作品缓存基础TTL：26天
    private static final long WORKS_BASE_TTL_DAYS = 26;
    ...
    private void syncDataFromMysqlToRedis() {
        try {
            ...
            for (Likes like : allLikes) {
                ...
                // 设置较长的过期时间，避免频繁同步
                redisUtil.expireWithJitter(redisKey, 30, TimeUnit.DAYS);
            }
    ...
    public Result getRouteGuides(Integer routeId) {
        try {
            ...
            if (guidesList != null) {
                redisUtil.expireWithJitter(redisKey, 1, TimeUnit.MINUTES);
        ...
        try {
            if (oldStatus != null) {
                ...
                if (oldWorks != null) {
                    ...
                    redisUtil.setObjWithJitter(oldRedisKey, oldWorks, WORKS_BASE_TTL_DAYS, TimeUnit.DAYS);
                }
            }
            ...
            if (addToNew) {
                ...
                if (!exists) {
                    newWorks.add(route);
                    redisUtil.setObjWithJitter(newRedisKey, newWorks, WORKS_BASE_TTL_DAYS, TimeUnit.DAYS);
                }
            }
        } catch (Exception e) {
        ...
        try {
            ...
            redisTemplate.execute((org.springframework.data.redis.core.RedisCallback<Void>) connection -> {
                var cursor = connection.scan(
                        org.springframework.data.redis.core.ScanOptions.scanOptions().match("user_works:*").count(100).build());
            ...
            for (String key : keys) {
                ...
                if (works != null && !works.isEmpty()) {
                    ...
                    if (removed) {
                        redisUtil.setObjWithJitter(key, works, WORKS_BASE_TTL_DAYS, TimeUnit.DAYS);
