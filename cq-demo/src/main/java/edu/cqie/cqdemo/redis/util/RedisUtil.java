...
public class RedisUtil {
    ...
    /**
     * 设置对象缓存（带随机抖动过期时间）
     * 用于 RedisTemplate<String, Object> 场景
     */
    public void setObjWithJitter(String key, Object value, long baseTimeout, TimeUnit unit, double jitterFactor) {
        long jitter = (long) (baseTimeout * jitterFactor);
        long randomJitter = random.nextLong(jitter * 2 + 1) - jitter;
     ...
     * 设置对象缓存（使用默认抖动因子 10%）
     */
    public void setObjWithJitter(String key, Object value, long baseTimeout, TimeUnit unit) {
        setObjWithJitter(key, value, baseTimeout, unit, DEFAULT_JITTER_FACTOR);
    }

    /**
     * 向 Set 添加成员（带抖动TTL，防雪崩）
     * 首次添加时设置过期时间，后续追加不重置 TTL
     */
    public void addToSetWithJitter(String key, Object value, long baseTimeout, TimeUnit unit) {
        Boolean hasKey = redisTemplate.hasKey(key);
        redisTemplate.opsForSet().add(key, value);
        if (Boolean.FALSE.equals(hasKey)) {
            long jitter = (long) (baseTimeout * DEFAULT_JITTER_FACTOR);
            long randomJitter = random.nextLong(jitter * 2 + 1) - jitter;
            long finalTimeout = Math.max(1, baseTimeout + randomJitter);
    ...
    /**
     * 设置过期时间（带随机抖动，防雪崩）
     * 用于替换 redisTemplate.expire(key, timeout, unit) 的固定TTL
     */
    public void expireWithJitter(String key, long baseTimeout, TimeUnit unit) {
        long jitter = (long) (baseTimeout * DEFAULT_JITTER_FACTOR);
        long randomJitter = random.nextLong(jitter * 2 + 1) - jitter;
        long finalTimeout = Math.max(1, baseTimeout + randomJitter);
        redisTemplate.expire(key, finalTimeout, unit);
     ...
     * 判断对象 key 是否存在
     */
    public boolean hasObjKey(String key) {
     ...
     * 一次操作涉及多个 key 时使用
     */
    public void delayDoubleDeleteMultiKey(String... keys) {
        for (String key : keys) {
            stringRedisTemplate.delete(key);
        }
        hotKeyExecutor.execute(() -> {
     ...
     * 延迟双删（带数据库操作 + 自定义延迟时间）
     */
    public void updateWithDelayDoubleDelete(String key, Runnable dbUpdater, long delayMs) {
        stringRedisTemplate.delete(key);
        dbUpdater.run();
        hotKeyExecutor.execute(() -> {
            try {
     ...
     * 删除后延迟从 DB 重新加载数据到缓存
     */
    public void delayDoubleDeleteAndRebuild(String key, Runnable dbUpdater,
